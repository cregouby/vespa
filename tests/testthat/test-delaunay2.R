...
test_that("delaunay2 rejects non-2-column input", {
  expect_error(delaunay2(matrix(1:9, 3, 3)), class = "rlang_error")
})

test_that("delaunay2 rejects non-numeric input", {
  expect_error(delaunay2(data.frame(x = "a", y = "b")), class = "rlang_error")
})
